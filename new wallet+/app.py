...
app = Flask(__name__)
app.secret_key = 'walletplus_super_secret_key_2026'
...
def get_external_wallet_balances(address):
    ...
    Currently we do not have on-chain RPC access, so we return zeros.
    The real address is stored. In a production version you would query
    the actual chain(s) using the address (via Alchemy, QuickNode, Moralis, etc.).
    """
    if not address:
        return {}
    # No fake balances — real amounts would come from blockchain queries.
    return {
        'bitcoin':  0.0,
        'ethereum': 0.0,
        'solana':   0.0,
...
def ensure_user_has_wallet_and_holdings(user_id):
    ...
    Returns list of dicts (not sqlite Row) so .get() and other dict methods are safe.
    """
    conn = get_db()
    ...
    if not wallets:
        ...
        wallet_id = conn.execute("SELECT last_insert_rowid()").fetchone()[0]
        wallets = conn.execute("SELECT * FROM wallets WHERE id = ?", (wallet_id,)).fetchall()
    ...
    conn.commit()
    conn.close()
    # Convert to plain dicts so template/JS and any .get() calls work safely
...
def user_send():
    try:
        ...
        if not holding or (holding['amount'] or 0) < amount:
            conn.close()
            return jsonify({'status': 'error', 'message': 'Insufficient balance'}), 400

        # Deduct
        new_amt = (holding['amount'] or 0) - amount
        conn.execute("UPDATE holdings SET amount = ? WHERE id = ?", (new_amt, holding['id']))

        # Log tx (reuse the open connection to avoid DB lock)
        ...
        conn.execute("INSERT INTO admin_logs (admin_id, action, target_user_id, details, timestamp) VALUES (?, ?, ?, ?, ?)",
                     (session['user_id'], 'user_send', user_id, json.dumps({'coin': coin_id, 'amount': amount, 'to': to_address[:20]}), datetime.now().isoformat()))
        conn.commit()
        conn.close()
...
if __name__ == '__main__':
    print("🚀 Wallet+ Starting... (Loading Screen Active)")
